...
if (_damage > 0.4) then { //0.25
	...
	if (_unit == player) then {
		...
		if ((_damage > 1.5) and _isHeadHit) then {
			_id = [_source,"shothead"] call fnc_usec_damageUnconscious;
		};
	};
...
if (_type == 1) then {
	...
	*/
	...
	if (_damage > 4) then {
		...
		if (_unit == player) then {
			_id = [_source,"explosion"] call fnc_usec_damageUnconscious;
		};
	} else {
		if (_damage > 2) then {
	...
	*/
	if (_damage > 4) then {
		...
		if (_unitIsPlayer) then {
			_id = [_source,"shotheavy"] call fnc_usec_damageUnconscious;
		};
	} else {
		if (_damage > 2) then {
			_isCardiac = _unit getVariable["USEC_isCardiac",false];
